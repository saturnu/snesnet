/*
 * virtual snesnet userspace device v0.1 beta
 * by saturnu 2015
 * 
 * If uhid is not available as /dev/uhid, then you can pass a different path as
 * first argument.
 * If <linux/uhid.h> is not installed in /usr, then compile this with:
 *   gcc -o ./vsnesnet -Wall -I./include ./vsnesnet.c
...
static int create(int fd)
{
	...
	ev.type = UHID_CREATE;
	strcpy((char*)ev.u.create.name, "vsnesnet-gamepad");
...
int read_joystick (int fd0)
{
	...
	if (!btnmapok) {
		...
		puts("vsnesnet is not fully compatible with your kernel. Unable to retrieve button map!");
		...
		while (1) {
			...
			if(js.type==js_axis_type || js.type==js_button_type){
					...
					if(ready){
						...
						//send button to snesnet opponent
						sendto(_SNESnet_socket_tcp,buffer,strlen(buffer),0,
						(struct sockaddr *)&_SNESnet_server_addr,sizeof(struct sockaddr_in) );
						//only slave mode
						//TODO master mode
...
int process_incoming(uint8_t a, uint8_t b){
		...
		btn13_down = !(b & (1 << 0)); //B 12
		btn14_down = !(b & (1 << 1)); //Y 13
		btn15_down = !(b & (1 << 2)); //Sl 14
		btn16_down = !(b & (1 << 3)); //St 15
		btn17_down = !(b & (1 << 4)); //Up 16
		btn18_down = !(b & (1 << 5)); //Do 17
		btn19_down = !(b & (1 << 6)); //Le 18
		btn20_down = !(b & (1 << 7)); //Ri 19

		btn21_down = !(a & (1 << 0)); //A 20
		btn22_down = !(a & (1 << 1)); //X 21
		btn23_down = !(a & (1 << 2)); //L 22
		btn24_down = !(a & (1 << 3)); //R 23
